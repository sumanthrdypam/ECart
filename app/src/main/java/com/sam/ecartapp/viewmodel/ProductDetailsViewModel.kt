...
class ProductDetailsViewModel(application: Application): AndroidViewModel(application){
    ...
    fun getDetailedProductResponse(id:String){
        apiService.getDetailedProductDetails(id).enqueue(object: Callback<ProductDetailedResponse>{
    ...
    fun onItem(position: String, quantity: Int) {
        ...
        val product = Product(productDetails.averageRating,productDetails.categoryId,"",productDetails.description,productDetails.price,
        ...
        if(quantity>0){
            ...
            val cartUsed = appDatabase.cartDao().findCartItemById(product.productId)
            if(cartUsed!=null && cartUsed.quantity>0){
                cartUsed.quantity +=cartUsed.quantity
                appDatabase.cartDao().insertCart(cartUsed)
            }
        }else{
